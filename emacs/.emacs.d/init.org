...
the function =org-babel-tangle=. =org-babel-tangle= will need to be
called manually for first-time setup.

The code which compiles ~init.org~ to ~init.el~ is in located above
this preface but may not be visible if you are viewing a rendered
